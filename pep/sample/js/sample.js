...
$(document).ready(function(){
  ...
  $("#subscription-dialog").dialog({
      ...
      modal: true,
      width: 400,
      title: "PEP Subscription",
      buttons: {
	"Subscribe": sample.subscribe ,	
	"Unsubscribe": sample.unsubscribe,	
	"Cancel": sample.closeSubscriptionDialog
      }
  });
  
  $("#pepSubscriptionButton").button().click(function(){
    $("#subscription-dialog").dialog("open");
  });
  
  $("#logoutButton").button().click( sample.logout );
  $("#publishButton").button().click( sample.publish );
